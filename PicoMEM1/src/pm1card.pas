unit pm1card;
{ PM1CARD -- talking to a PicoMEM 1.x card from real-mode DOS.
  PicoMEM1 tools, StevenC.  Public domain (the Unlicense).

  A sibling of PicoMEM2's pmcard.pas, and it differs in two ways that
  the PicoMEM 1 forced:
  ...
  * THE PARAMETER AREA MOVED.  Text answers land in a 2 KB area in the
    ...
    tool in PicoMEM2 that reads a text answer therefore reads nothing
    at all on this card -- PMPROBE reports "USB: 0 line(s)" whatever is
    plugged in.  FindParam below finds the area instead of assuming it.
...
{ FPC's Intr LOADS Registers.Flags into the CPU before the INT, and the
  ...
  wedged the PicoMEM2 tools three times before it was understood. }
function AskBios: Boolean;
...
begin
  case R of
    CR_OK:       ResultName := 'ok';
    CR_REFUSED:  ResultName := 'refused by PM1CARD (not whitelisted)';
