program pm1cfg;
{ PM1CFG -- the whole configuration a PicoMEM 1 is running.
  ...
  (hold a key at boot); nothing here writes, and pm1card.pas would
  ...
  area sits -- see PM1INFO -- so the extra fields are printed only when
...
begin
  ...
  WriteLn('  WiFi enable flag: ', CfgB(C_ENABLEWIFI),
          '   (PM1INFO reports whether it actually came up)');
end;
...
begin
  Args;
  WriteLn('PM1CFG ', VER, ' -- the configuration this PicoMEM is running');
