...
{ PMMTEST -- does a USB mouse on the card reach the PC at all?
  ...
  off again.
    ...
    /L   log each phase to C:\WORK\PMMTEST.LOG, closed after every
...
const
  ...
  { pm_irq_svar_t, the IRQ mux's shared variables.  It sits 32 bytes
    ...
    area PMCARD found keeps it right on either firmware. }
...
{ A siren, not a polite pair of pips.  The first version was two short
  ...
  then a pause to get a hand in place.  PMWATCH uses the same two. }
procedure BeepStart;
var I: Integer;
begin
...
begin
  ...
  if not FindParam then begin
    ...
    WriteLn('be located either -- see PMINFO.');
  ...
  if Events > 0 then begin
    ...
    if DoEnable then begin
      ...
      WriteLn('not claim it (PMSTAT says), or reporting needs more than the');
