...
{ PMSTAT -- the answers a PicoMEM will actually give you.
  ...
  firmware, so this finds it first (see pmcard.pas) rather than
  assuming -- which is why the first version of these tools reported
  nothing at all on a card with the 2025-11-02 BIOS.
...
begin
  ...
  WriteLn('  the network key is in the same structure, 63 bytes at +',
          W_KEY, ';');
  WriteLn('  this tool never reads them, and PMDUMP /K blanks them.');
end;
