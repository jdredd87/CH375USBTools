...
{ PMINFO -- what is this PicoMEM, and how did it come up?
  ...
  the parameter area is located (pmcard.pas says why it has to be).
...
begin
  ...
  if (B = 0) and (P = 0) and (W = 0) then begin
    ...
    { The revision word is zero on both firmwares seen so far -- the
      ...
      which PMINFO prints above, is the identifier that works. }
    if W = 0 then
...
begin
  ...
  { The firmware's own header says of this byte: "Bit 0: Mouse Bit 1:
    ...
    still leaves this 00, as PMMTEST demonstrates.  Printed because it
